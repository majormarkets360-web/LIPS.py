...
with st.sidebar:
    ...
    chain_options = ["ethereum", "base", "arbitrum", "solana", "bsc", "polygon"]
    selected_chains = st.multiselect("Filter chains", chain_options, default=["ethereum"])

    st.divider()
    auto_refresh = st.toggle("Auto-refresh", value=True)
    refresh_sec = st.slider("Refresh interval (seconds)", 10, 60, 15)
...
if st.button("🔄 Refresh Now", type="primary") or "raw_pairs" not in st.session_state:
    with st.spinner("Fetching live pool data..."):
        st.session_state.raw_pairs = fetch_pairs(query)
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["📋 All Pools", "📊 Charts by DEX", "📈 Live History", "⚡ Price Impact Simulator"]
)
...
with tab1:
    ...
    st.caption(
        f"Last updated: {st.session_state.get('last_update', '—')}  •  {len(df)} pools"
    )
...
with tab2:
    ...
    with colB:
        agg = (
            df.groupby("DEX")
            .agg({"Liquidity USD": "sum", "24h Volume": "sum"})
            .reset_index()
        )
        st.plotly_chart(
            px.bar(agg, x="DEX", y="Liquidity USD", title="Total Liquidity per DEX"),
            use_container_width=True,
        )
        st.plotly_chart(
            px.bar(agg, x="DEX", y="24h Volume", title="24h Volume per DEX"),
            use_container_width=True,
        )
...
with tab3:
    ...
    pool_options = df["Pair Address"].dropna().unique().tolist()

    def _label(addr):
        match = df[df["Pair Address"] == addr]
        return match["Pair"].iloc[0] if not match.empty else addr
    ...
    # Update history on every render
    for addr in selected_addresses:
        ...
        st.session_state.pool_history[addr].append(
            (datetime.now(), row["Price USD"], row["Liquidity USD"])
        )
        ...
        for addr in selected_addresses:
            ...
            if len(hist) >= 2:
                ...
                hist_df = pd.DataFrame(
                    {"Price (USD)": prices, "Liquidity (USD)": liqs}, index=times
                )
                st.line_chart(hist_df, use_container_width=True)
                pair_label = df[df["Pair Address"] == addr]["Pair"].iloc[0]
                dex_label  = df[df["Pair Address"] == addr]["DEX"].iloc[0]
                st.caption(f"Pool: {pair_label} on {dex_label}")
            else:
                st.info("Need at least 2 refreshes to draw a chart — keep auto-refresh on.")

# ── TAB 4: Price Impact Simulator ────────────────────────────────────────────
with tab4:
    ...
    if df.empty:
        ...
        selected_idx = st.selectbox(
            ...
            format_func=lambda i: (
                f"{df.iloc[i]['DEX']} — {df.iloc[i]['Pair']} "
                f"(${df.iloc[i]['Liquidity USD']:,.0f} liquidity)"
            ),
        )
        ...
        direction = st.radio(
            "Swap direction",
            [f"{base_sym} → {quote_sym}", f"{quote_sym} → {base_sym}"],
        )
        ...
        if direction.startswith(base_sym):
            reserve_in, reserve_out = row["Base Liquidity"], row["Quote Liquidity"]
            token_in,   token_out   = base_sym, quote_sym
        else:
            reserve_in, reserve_out = row["Quote Liquidity"], row["Base Liquidity"]
            token_in,   token_out   = quote_sym, base_sym
        ...
        if reserve_in > 0 and reserve_out > 0:
            ...
            col1.metric("Price Impact", f"{price_impact_pct:.2f}%")
            col2.metric("Amount Out",   f"{amount_out:,.4f} {token_out}")
            st.info(
                f"Spot price: 1 {token_in} = {spot_price:,.4f} {token_out}\n\n"
                f"Effective price after impact: 1 {token_in} = {execution_price:,.4f} {token_out}"
            )
        else:
            st.warning("Insufficient reserve data for this pool — try another.")
